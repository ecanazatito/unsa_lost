...
def visualize_predictions(image, pred, seed, scales, dims, vis_folder, im_name, plot_seed=False):
    ...
    pltname = f"{vis_folder}/{im_name}_LOST.png"
...
def visualize_fms(A, seed, scores, dims, scales, output_folder, im_name):
    ...
    im_corr = np.zeros((3, len(scores)), dtype=np.float32)
    where = binA[seed, :] > 0
    ...
    # Save correlations
    save_path_corr = f"{output_folder}/{im_name}_corr.png"
    try:
        skimage.io.imsave(
            fname=save_path_corr,
            arr=im_corr.transpose((1, 2, 0)),  # Transpose for correct channel order
        )
        print(f"Image saved at {save_path_corr}.")
    except Exception as e:
        print(f"Failed to save {save_path_corr}: {e}")
    ...
    # Convert to uint8 for saving as image
    im_deg = (im_deg * 255).astype(np.uint8)

    # Save inverse degree map
    save_path_deg = f"{output_folder}/{im_name}_deg.png"
    try:
        plt.imsave(fname=save_path_deg, arr=im_deg, cmap='viridis')
        print(f"Image saved at {save_path_deg}.")
    except Exception as e:
        print(f"Failed to save {save_path_deg}: {e}")
...
def visualize_seed_expansion(image, pred, seed, pred_seed, scales, dims, vis_folder, im_name):
    ...
    pltname = f"{vis_folder}/{im_name}_LOST_seed_expansion.png"
    Image.fromarray(image).save(pltname)
